...
- A free Vercel account *(deployment only)*
...
2. From **Settings → API**, copy: `Project URL`, `anon` key, `service_role` key.
...
2. `supabase/migrations/0002_seed_reference.sql`

### 5. Seed demo data
```bash
npm run seed
```
This creates 7 demo users (3 employees · 2 managers · 1 admin · 1 HR), sheets in every workflow state, sample check-ins, escalations, and notifications.
...
3. (Optional) Add `vercel.json` cron config below to run nightly escalation sweeps
...
## 📊 Evaluation criteria — how this submission scores

| Parameter | How this build wins |
|-----------|---------------------|
| **Functionality** | All Phase 1 + Phase 2 features implemented as a real system, not a mockup. Role separation enforced at the database layer. |
| **BRD adherence** | Validation rules (weightage = 100%, min 10%, max 8 goals) and all four scoring formulas live in `lib/goals.ts` as pure functions — open the file and check them against the BRD line-by-line. |
| **User-friendliness** | Refined dark theme. Instant client-side validation. Live weightage donut. One-click SSO for demos. Optimistic UI. |
| **Bug-free experience** | Server actions revalidate Next.js cache on every mutation. RLS prevents privilege escalation even if a client request is tampered with. |
| **Bonus features** | All four §5 bonuses shipped (SSO, notifications, escalations, analytics). |
| **Cost optimisation** | $0/mo on free tier — full architecture rationale above. |
